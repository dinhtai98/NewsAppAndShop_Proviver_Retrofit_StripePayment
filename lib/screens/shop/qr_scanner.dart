...
class QRViewScanner extends StatefulWidget {
  static const routeName = '/qRViewScanner';
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) {
      setState(() {
        result = scanData;
        if (result != null) {
          ServiceSearchDataProduct.getData(result.code).then((value) {
            p = value;
          });
        }
      });
        ...
        Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) => DescriptionProduct(
                        arguments:
                            DescriptionProductScreenArguments(product: p))))
            .then((ctx) => controller?.resumeCamera());
        // controller?.resumeCamera();
      }
    });
  }
